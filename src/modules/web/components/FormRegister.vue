...
import { InputUser, LEVEL_ACCESS, TYPE_REGISTER } from 'src/modules/register/contracts';
...
const save = async () => {
  ...
  try {
    ...
    /**
     * check origin for action
     */
    const originRouterName = route.name as string;
    if (originRouterName === 'webRegister') {
      router.push({ name: 'auth' });
    }

  } catch (error) {
    notification.error('Erro ao realizar cadastro!');
  } finally {
    hideLoading();
  }
};
